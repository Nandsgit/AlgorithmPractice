...
public class ActivitySelection {
    ...
    public List<Activity> selectActivitiesDynamic_listMaxSumWeights() {
        ...
        for(int j = 1; j<=n; j++){
            ...
            if (weightsumIncluding > weightsumExcluding)
                choosingList[j - 1] = 1;
            }

        int chooseActivity = choosingList.length - 1;

        while(chooseActivity >= 0){
            if (choosingList[chooseActivity] == 0){
                chooseActivity--;
            } else {
                maxWeightList.add(0, activities.get(chooseActivity));
                chooseActivity = lastfinishingjob[chooseActivity];
            }
        }
